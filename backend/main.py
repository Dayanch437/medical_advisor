...
from models import MedicalQuestion, MedicalAdvice, HealthStatus, QueryHistory, QueryHistoryItem
from config import get_settings
import logging
from sqlalchemy.ext.asyncio import AsyncSession
from database import get_db, init_db
from db_models import MedicalQuery, AIResponse
...
# Lifespan event handler (databazany işe girizmeк)
@asynccontextmanager
async def lifespan(app: FastAPI):
    """Programma başlanda we gutaranda işlär"""
    # Startup
    await init_db()
    logger.info("Databaza üstünlikli işe girizildi")
    yield
    # Shutdown (gerek bolsa)
...
async def get_medical_advice(question: MedicalQuestion, db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        db.add(db_query)
        await db.flush()  # ID almak üçin
        
        # AI jogabyny saklamak
...
async def get_query_history(
    ...
    """
    try:
        # Ähli soraglary we jogaplary almak
        stmt = (
        ...
        # Umumy sany almak
        count_stmt = select(MedicalQuery)
        count_result = await db.execute(count_stmt)
        total = len(count_result.all())
        
        # QueryHistoryItem listini döretmeк
        history_items = []
